...
<?php //dd($functions->hash_pass("aline1382")); ?>
<?php 
...
if (isset($_POST["submit"])) {
    ...
    $number = $_POST["number"];
    if (empty($number)) {
        array_push($errors,"شماره موبایل خود را وارد کنید");
    }
    ...
    if (empty($errors)) {
        $user = DB::table("users")->where("mobile_number",$number)->where("password",$hashed_password)->get();
        $last_login = $functions->get_date();
        if ($user->Count() > 0) {
            DB::table('users')->where("mobile_number", $number)->update(['last_login' => $last_login]);
            Cookie::queue(Cookie::make("users_access",$user[0]->id,300*10));
            ...
            <div class="row align-items-center justify-content-between pt-5 pt-sm-5 pt-md-5 pt-lg-0">
                <div class="col-md-7 col-lg-6">
                    <div class="hero-content-left text-white">
                        <h1 class="text-white">خوش برگشتی</h1>
                        <p class="lead">
                ...
                <div class="col-md-5 col-lg-5">
                    <div class="card login-signup-card shadow-lg mb-0">
                        <div class="card-body px-md-5 py-5">
                            ...
                            <!--login form-->
                            <form class="login-signup-form" action="/users/login" method="POST">
                            @csrf    
                            <div class="form-group">
                                    <label class="pb-1">شماره همراه</label>
                                    <div class="input-group input-group-merge">
                                        <div class="input-icon">
                                            <span class="ti-email color-primary"></span>
                                        </div>
                                        <input type="text" name="number" class="form-control" value="<?php echo $this_user_info[0]; ?>" placeholder="شماره همراه:">
                                    </div>
                                </div>
                                <!-- Password -->
                                <div class="form-group">
